export const projects = [
  {
    id: 3,
    imageSrc: require("../../public/images/3.png"),
    title: "Amma The Handmade Spices",
    description:
      "   I combined Bootstrap and React custom hooks to craft an efficient application, showcasing impactful cart features and skillful state  management for a seamless user experience. Used React  custom hooks for efficient state management and easy flow of code.",
    githubLink: "https://www.ammahandmadespices.com/",
    demoLink: "https://www.ammahandmadespices.com/",
    techStack: ["React", "ReactStrap"],
  },

  {
    id: 2,
    imageSrc: require("../../public/images/2.png"),
    title: "Sai sanjeevani yoga",
    description:
      "Led the end-to-end development and deployment of saisanjeevaniyoga.in, utilizing HTML, CSS, and JavaScript, achieving a 30% increase in user engagement through performance optimization and responsive design.Successfully managed the project lifecycle, from conceptualization to deployment on Vercel.",
    githubLink: "https://www.saisanjeevaniyoga.in/",
    demoLink: "https://www.saisanjeevaniyoga.in/",
    techStack: ["HTML", "CSS", "Boostrap"],
  },
  {
    id: 1,
    imageSrc: require("../../public/images/1.png"),
    title: "Bharghav Self Drive Cars",
    description:
      " Crafted a dynamic Digital Marketing Landing Page that succinctly communicates diverse services and drives lead generation through engaging visuals and responsive design. Client success showcases further validate credibility, paving the way for business growth",
    githubLink: "https://bharghavselfdrivecarsvjwd.in/",
    demoLink: "https://bharghavselfdrivecarsvjwd.in/",
    techStack: ["HTML", "CSS", "Bootstrap"],
  },
];
